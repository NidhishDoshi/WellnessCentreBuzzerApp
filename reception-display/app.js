...
const CALL_TYPE = 'reception'; // Filter for reception calls only
...
socket.on('newReceptionCall', (call) => {
    ...
    if (call.type === 'reception') {
        ...
        console.log('🎵 Triggering sound playback for new reception call...');
        playNotificationSound();
...
socket.on('newNurseCall', (call) => {
    console.log('New nurse call received (ignored):', call);
    // Reception display ignores nurse calls
});
...
// Render calls (filtered for reception type only)
function filterAndRenderCalls() {
    receptionCalls = allCalls.filter(call => call.type === 'reception');
    
    if (receptionCalls.length === 0) {
        showCarousel();
        return;
    }
    ...
    callsGrid.setAttribute('data-count', receptionCalls.length);
    
    // Sort calls by timestamp (oldest first, so they get attention)
    const sortedCalls = [...receptionCalls].sort((a, b) => 
...
// Play notification sound - Web Audio API ONLY (no HTML5 autoplay restrictions)
function playNotificationSound() {
    ...
    try {
        console.log('🔔 Playing reception alert sound...');
...
// Show browser notification for new reception calls
socket.on('newReceptionCall', (call) => {
    if ('Notification' in window && Notification.permission === 'granted') {
        new Notification('Reception Call Request', {
